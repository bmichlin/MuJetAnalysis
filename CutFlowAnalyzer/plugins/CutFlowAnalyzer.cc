...
private:
  ...
  Float_t m_orphan_isoTk_nodz;
  Float_t m_orphan_isoTk_no05;
  Float_t m_orphan_dimu_isoTk;
  Float_t m_orphan_dimu_isoTk_nodz;
...
CutFlowAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(runBBestimation_){
    ...
    m_orphan_mass = -999.;
    m_orphan_isoTk = -999.;
    m_orphan_isoTk_nodz = -999.;
    m_orphan_isoTk_no05 = -999.;
    m_orphan_dimu_isoTk = -999.;
    m_orphan_dimu_isoTk_nodz = -999.;
    m_orphan_z = -999.;
    m_orphan_dimu_z = -999.;
    ...
    if (muJets->size() == 1  &&  (*muJets)[0].numberOfDaughters() == 2  &&  orphans->size() == 1 ) {
      ...
      m_orphan_isoTk = 0.;
      m_orphan_isoTk_nodz = 0.;
      m_orphan_isoTk_no05 = 0.;
      m_orphan_dimu_isoTk = 0.;
      m_orphan_dimu_isoTk_nodz = 0.;
      for (reco::TrackCollection::const_iterator track = tracks->begin(); track != tracks->end(); ++track) {
        if (!muJet->sameTrack(&*track,&*(orphan->innerTrack()))) {
          ...
          if (dz < 0.1 && dR < 0.4) m_orphan_isoTk_no05 += track->pt();
          if (dR < 0.4 && track->pt() > iso_track_pt_treshold) {
            if (dz < 0.1){ m_orphan_isoTk += track->pt(); }
            m_orphan_isoTk_nodz += track->pt();
          }
        }
      }
      //iso dimuon-orphan
       for (reco::TrackCollection::const_iterator track = tracks->begin(); track != tracks->end(); ++track) {
         ...
         if (!track_is_muon) {
           ...
           if (dR < 0.4 && track->pt() > iso_track_pt_treshold) {
             ...
             m_orphan_dimu_isoTk_nodz += track->pt();
           }
         }    
       }
     }
...
CutFlowAnalyzer::beginJob() {
  ...
  if(runBBestimation_){
    ...
    m_ttree_orphan->Branch("orph_isoTk_nodz", &m_orphan_isoTk_nodz, "orph_isoTk_nodz/F");
    m_ttree_orphan->Branch("orph_isoTk_no05", &m_orphan_isoTk_no05, "orph_isoTk_no05/F");
    m_ttree_orphan->Branch("orph_dimu_isoTk", &m_orphan_dimu_isoTk, "orph_dimu_isoTk/F");
    m_ttree_orphan->Branch("orph_dimu_isoTk_nodz", &m_orphan_dimu_isoTk_nodz, "orph_dimu_isoTk_nodz/F");
