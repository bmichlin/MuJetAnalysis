...
class CutFlowAnalyzer : public edm::EDAnalyzer {
	...
	private:
		...
		TTree * m_ttree_jpsi; //Store variables in branches of this tree for later access  
		...
		bool runPromptJpsiEstimation_; 

		bool b_isjpsi;
		bool b_is2MuJetsjpsi;
		bool b_is2DiMuonsFittedVtxOKjpsi;
		bool b_is2DiMuonsIsoTkOK_FittedVtxjpsi;
		bool b_is2DiMuonsjpsi;
		bool b_isoC_1mmjpsi;
		bool b_isoF_1mmjpsi;
		bool b_diMuonC_IsoTk_FittedVtxjpsi;
		bool b_diMuonF_IsoTk_FittedVtxjpsi;
		Int_t m_events2DiMuonsjpsi;
		Int_t m_events2MuJetsjpsi;
		Bool_t b_is2DiMuonsDzOK_FittedVtx_jpsi;
		Int_t  m_events2DiMuonsDzOK_FittedVtx_jpsi;   // ... with dz between dimuons less than 0.1
		Float_t b_diMuons_dz_FittedVtx_jpsi;
		Float_t b_diMuonC_FittedVtx_dz_jpsi;
		Float_t b_diMuonF_FittedVtx_dz_jpsi;
...

{
	...
	m_randomSeed = rand() % 9999 + 1000; //Generate a random number between 1000 and 9999 
...
const pat::MultiMuon *muJetC = NULL;
	...
	//SKIP THE HLT CUT FOR LUCA'S JPSI DPS SAMPLE
...
std::cout << "Checkpoint 4.2" << std::endl;
		jpsi_event_failure_type = 0;
std::cout << "Checkpoint 4.3" << std::endl;


		//For data

		//See if the event fired the trigger
...
if(is_jpsi_mc == false){
	if( triggerEvent->path("HLT_Dimuon0_Jpsi_Muon_v2")->wasAccept() && trigger == 0 ){
		...
		trigger++;
		std::cout << "This event fired the HLT_Dimuon0_Jpsi_Muon_v2 trigger!" << std::endl;
		jpsi_trigger = 1;
	}
}
		std::cout << "Checkpoint 6" << std::endl;

		std::cout << "cleanPatPFMuons" << std::endl; 
		for (pat::MuonCollection::const_iterator iMuon = jpsiMuonCands->begin();  iMuon != jpsiMuonCands->end();  ++iMuon) {
				const pat::TriggerObjectStandAlone *trigger_mu  = iMuon->triggerObjectMatchByPath("HLT_Dimuon0_Jpsi_Muon_v2");
				if(trigger_mu != NULL) trigger++;
				if(trigger_mu != NULL && is_jpsi_mc == true) jpsi_trigger = 1;
				if(trigger_mu != NULL) std::cout << "This muon fired the HLT_Dimuon0_Jpsi_Muon_v2 trigger!" << std::endl;
				std::cout << "Muon pT: " << iMuon->pt() << std::endl;
				if ( iMuon->pt() > 3.5 ) {
					jpsi_selMuons.push_back( &(*iMuon) );
					standardMuons++;				
				}
		}

		std::cout << "patMuons" << std::endl;
		for (pat::MuonCollection::const_iterator iMuon = jpsiMuonCandsPAT->begin();  iMuon != jpsiMuonCandsPAT->end();  ++iMuon) {
				//const pat::TriggerObjectStandAlone *trigger_mu  = iMuon->triggerObjectMatchByPath("HLT_Dimuon0_Jpsi_Muon_v2");
				//if(trigger_mu != NULL) trigger++;
				//if(trigger_mu != NULL) std::cout << "This muon fired the HLT_Dimuon0_Jpsi_Muon_v2 trigger!" << std::endl;
				std::cout << "Muon pT: " << iMuon->pt() << std::endl;
		...
		for (reco::MuonCollection::const_iterator iMuon = jpsiMuonCandsRECO->begin();  iMuon != jpsiMuonCandsRECO->end();  ++iMuon) {
				//const pat::TriggerObjectStandAlone *trigger_mu  = iMuon->triggerObjectMatchByPath("HLT_Dimuon0_Jpsi_Muon_v2");
				//if(trigger_mu != NULL) trigger++;
				//if(trigger_mu != NULL) std::cout << "This muon fired the HLT_Dimuon0_Jpsi_Muon_v2 trigger!" << std::endl;
				std::cout << "Muon pT: " << iMuon->pt() << std::endl;
		...
		//Do this in another way and compare

/*
		for (pat::MuonCollection::const_iterator muon = muons->begin();  muon != muons->end();  ++muon) {
			const pat::TriggerObjectStandAlone *mu01  = muon->triggerObjectMatchByPath("HLT_Dimuon0_Jpsi_Muon_v2");
			if(mu01 != NULL) trigger++;
			if(muon->pt() > 3.5){
				//jpsi_selMuons.push_back(muon);
				bbarMimickMuons++;
			}
		}
*/

		if(standardMuons != patMuons || standardMuons != recoMuons || patMuons != recoMuons ) std::cout << "Watch this event" << std::endl;
		std::cout << "Number of each type of muon (RECO, PAT, CleanPatPF, bbar): " << recoMuons << ", " << patMuons << ", " << standardMuons << ", " <<  std::endl;
...
*/
		...
		//
		// if(trigger != 0) jpsi_event_failure_type++;
		//if(trigger != 0 || is_jpsi_mc) jpsi_event_failure_type++;
		if(trigger != 0 || trigger == 0 || is_jpsi_mc) jpsi_event_failure_type++;
		//if(trigger !=0 && jpsi_selMuons.size() >=4) jpsi_event_failure_type++;
		if((trigger !=0 || trigger == 0 || is_jpsi_mc) && jpsi_selMuons.size() >=4) jpsi_event_failure_type++;

		//Initialize all variables here
		b_is2DiMuonsIsoTkOK_FittedVtxjpsi = false;
		jpsi_isoTk_dimuon1 = -1.;
		jpsi_isoTk_dimuon2 = -1.;
		b_isoC_1mmjpsi = -1.;
		b_isoF_1mmjpsi = -1.;
		double diMuonC_IsoTk_FittedVtxjpsi = 0.0;
		double diMuonF_IsoTk_FittedVtxjpsi = 0.0;
		//if(trigger != 0)
		//TODO: Don't forget about this line (above)
		if(trigger != 0 || is_jpsi_mc || trigger ==0){
		//if(trigger != 0 || is_jpsi_mc)
			if ( m_debug > 10 ) std::cout << m_events << " Count Jpsi muons" << std::endl;
			std::cout << "Number of Jpsi muons: " << jpsi_selMuons.size() << std::endl;
			b_isjpsi = false;
			if ( jpsi_selMuons.size() >=4 ) {
				b_isjpsi = true;
			}
			...
			b_massCjpsi = -999.; b_massFjpsi = -999.;
			b_is2MuJetsjpsi = false;
			std::cout << "Number of Jpsi mu jets: " << nMuJetsjpsi << std::endl;
			if ( nMuJetsjpsi == 2) {
				b_is2MuJetsjpsi = true;
				if (m_trandom3.Integer(2) == 0) {
					muJetCjpsi = &((*JpsiMuJets)[0]);
					muJetFjpsi = &((*JpsiMuJets)[1]);
				} else {
					muJetCjpsi = &((*JpsiMuJets)[1]);
					muJetFjpsi = &((*JpsiMuJets)[0]);
				}
			}

			if ( m_debug > 10 ) std::cout << m_events << " Check if Jpsi mu jets are dimuons" << std::endl;
			if ( b_isjpsi && b_is2MuJetsjpsi) m_events2MuJetsjpsi++;
			if ( b_isjpsi && b_is2MuJetsjpsi) jpsi_event_failure_type++;
			b_is2DiMuonsjpsi = false;
			const pat::MultiMuon *diMuonCjpsi = NULL;
			const pat::MultiMuon *diMuonFjpsi = NULL;
			if ( muJetCjpsi != NULL && muJetFjpsi != NULL ) {
				if ( muJetCjpsi->numberOfDaughters() == 2 && muJetFjpsi->numberOfDaughters() == 2 ) {
					diMuonCjpsi = muJetCjpsi;
					diMuonFjpsi = muJetFjpsi;
					b_is2DiMuonsjpsi = true;
				}
			}
			if(b_is2DiMuonsjpsi){
				std::cout << "Both jpsi jets are dimuons"  << std::endl;
				jpsi_event_failure_type++;
			}else{
				std::cout << "Both jpsi jets are NOT dimuons"  << std::endl;
			}

			//if ( m_debug > 10 ) std::cout << m_events << " Check if 2 muon jpsi jets are dimuons" << std::endl;
			if ( b_isjpsi && b_is2MuJetsjpsi && b_is2DiMuonsjpsi){
				m_events2DiMuonsjpsi++;
				b_massCjpsi = muJetCjpsi->mass();
				b_massFjpsi = muJetFjpsi->mass();
			}
			// "Old" fitted vertexes
			std::cout << "Check if both dimuons have a valid vertex" << std::endl;				
			b_is2DiMuonsFittedVtxOKjpsi = false;
			if ( diMuonCjpsi != NULL && diMuonFjpsi != NULL ) {
				if ( diMuonCjpsi->vertexValid() && diMuonFjpsi->vertexValid() ) {
					b_is2DiMuonsFittedVtxOKjpsi = true;
				}
			}
			if(b_is2DiMuonsFittedVtxOKjpsi){
				std::cout << "Both dimuons have a valid vertex" << std::endl;
				jpsi_event_failure_type++;
			}else{
				std::cout << "Both dimuons do NOT have a valid vertex" << std::endl;
			}
			// Fill branches with variables calculated with "old" fitted vertexes
			if ( b_is2DiMuonsFittedVtxOKjpsi ) {
				std::cout << m_events << " Good jpsi vertices are found.  Filling dimuon variables. " << std::endl;
				jpsi_mass_dimuon1 = muJetCjpsi->mass();
				jpsi_mass_dimuon2 = muJetFjpsi->mass();
				std::cout << "jpsi_mass_1: " << jpsi_mass_dimuon1 << std::endl;
				std::cout << "jpsi_mass_2: " << jpsi_mass_dimuon2 << std::endl;
				jpsi_inv_mass_dimuon1 = diMuonCjpsi->vertexMass();
				jpsi_Lxy_dimuon1      = diMuonCjpsi->vertexLxy(beamSpotPosition);
				jpsi_L_dimuon1        = diMuonCjpsi->vertexL(beamSpotPosition);
				jpsi_px_dimuon1       = diMuonCjpsi->vertexMomentum().x(); 
				jpsi_py_dimuon1       = diMuonCjpsi->vertexMomentum().y(); 
				jpsi_pz_dimuon1       = diMuonCjpsi->vertexMomentum().z(); 

				jpsi_inv_mass_dimuon2 = diMuonFjpsi->vertexMass();
				jpsi_Lxy_dimuon2      = diMuonFjpsi->vertexLxy(beamSpotPosition);
				jpsi_L_dimuon2        = diMuonFjpsi->vertexL(beamSpotPosition);
				jpsi_px_dimuon2       = diMuonFjpsi->vertexMomentum().x(); 
				jpsi_py_dimuon2       = diMuonFjpsi->vertexMomentum().y(); 
				jpsi_pz_dimuon2       = diMuonFjpsi->vertexMomentum().z();

				//Working variables: Not included in nTuple
				b_diMuonC_FittedVtx_dz_jpsi  = diMuonCjpsi->vertexDz(beamSpot->position());
				b_diMuonF_FittedVtx_dz_jpsi  = diMuonFjpsi->vertexDz(beamSpot->position());

				std::cout << "jpsi_inv_mass_dimuon1: " << jpsi_inv_mass_dimuon1 << std::endl;
				std::cout << "jpsi_inv_mass_dimuon2: " << jpsi_inv_mass_dimuon2 << std::endl;
			} else {
				jpsi_mass_dimuon1 = -1000;
				jpsi_mass_dimuon2 = -1000;

				jpsi_inv_mass_dimuon1 = -1000.0; 
				jpsi_Lxy_dimuon1      = -1000.0;
				jpsi_L_dimuon1        = -1000.0;
				jpsi_px_dimuon1       = -1000.0;
				jpsi_py_dimuon1       = -1000.0;
				jpsi_pz_dimuon1       = -1000.0;

				jpsi_inv_mass_dimuon2 = -1000.0;
				jpsi_Lxy_dimuon2      = -1000.0;
				jpsi_L_dimuon2        = -1000.0;
				jpsi_px_dimuon2       = -1000.0;
				jpsi_py_dimuon2       = -1000.0;
				jpsi_pz_dimuon2       = -1000.0;

				b_diMuonC_FittedVtx_dz_jpsi  = -1000;
				b_diMuonF_FittedVtx_dz_jpsi  = -1000.0;

			}

			// Calculate dz between dimuons - use fitted vertexes
			b_is2DiMuonsDzOK_FittedVtx_jpsi = false;
			if ( b_is2DiMuonsFittedVtxOKjpsi ) {
				std::cout << "Calculate dz between Jpsi dimuons" << std::endl;
				b_diMuons_dz_FittedVtx_jpsi = b_diMuonC_FittedVtx_dz_jpsi - b_diMuonF_FittedVtx_dz_jpsi;
				if ( fabs( b_diMuons_dz_FittedVtx_jpsi ) < m_threshold_DiMuons_dz ){
					b_is2DiMuonsDzOK_FittedVtx_jpsi = true;
					std::cout << "Jpsi dimuons pass dz cut" << std::endl;
				}
			} else {
				b_diMuons_dz_FittedVtx_jpsi = -1000.0;
			}
			...
			/*
			   ...
			   double diMuonC_IsoTk_FittedVtxjpsi = 0.0;
			   double diMuonF_IsoTk_FittedVtxjpsi = 0.0;
			   double diMuonTmp_IsoTk_FittedVtxjpsi = 0.0;
			   */
			if(b_is2DiMuonsDzOK_FittedVtx_jpsi){
				jpsi_event_failure_type++;
				...
				//  iEvent.getByLabel("particleFlow", pfCandidates);
				// Cut on isolation - use fitted vertexes
				//if ( b_is2DiMuonsFittedVtxOKjpsi ) { //This is a bit redundant because the dz cut has already bee implemented

				//double diMuonC_IsoTk_FittedVtxjpsi = 0.0;
				//double diMuonF_IsoTk_FittedVtxjpsi = 0.0;

				const pat::MultiMuon *diMuonTmpjpsi = NULL;
				for ( unsigned int i = 1; i <= 2; i++ ) { 
					double diMuonTmp_IsoTk_FittedVtxjpsi = 0.0;
					if ( i == 1 ) {
						std::cout << "Calculating isolation for Dimuon 1..." << std::endl;
						diMuonTmpjpsi = diMuonCjpsi;

					}
					if ( i == 2 ) {
						diMuonTmpjpsi = diMuonFjpsi;
						std::cout << "Calculating isolation for Dimuon 2..." << std::endl;
					}
					for (reco::TrackCollection::const_iterator track = tracksjpsi->begin(); track != tracksjpsi->end(); ++track) {
						bool trackIsMuonjpsi = false;
						if (    diMuonTmpjpsi->sameTrack( &*track, &*(diMuonTmpjpsi->muon(0)->innerTrack()) )
								|| diMuonTmpjpsi->sameTrack( &*track, &*(diMuonTmpjpsi->muon(1)->innerTrack()) ) ) trackIsMuonjpsi = true;
						if ( trackIsMuonjpsi == false ) {
							double dPhijpsi = tamu::helpers::My_dPhi( diMuonTmpjpsi->vertexMomentum().phi(), track->phi() );
							double dEtajpsi = diMuonTmpjpsi->vertexMomentum().eta() - track->eta();
							double dRjpsi   = sqrt( dPhijpsi*dPhijpsi + dEtajpsi*dEtajpsi ); 
							double dzjpsi   = diMuonTmpjpsi->vertexDz(beamSpot->position()) - track->dz(beamSpot->position());
							if (    dRjpsi          < m_threshold_DiMuons_Iso_dR
									&& track->pt() > m_threshold_DiMuons_Iso_pT
									&& fabs( dzjpsi )  < m_threshold_DiMuons_Iso_dz ) {
								diMuonTmp_IsoTk_FittedVtxjpsi += track->pt();
								std::cout << "Track added to isolation.  Total isolation: " << diMuonTmp_IsoTk_FittedVtxjpsi << std::endl;
							}    
						}
					}
					//      for (reco::PFCandidateCollection::const_iterator pfCand = pfCandidates->begin(); pfCand != pfCandidates->end(); ++pfCand) 
					if ( i == 1 ) {
						diMuonC_IsoTk_FittedVtxjpsi = diMuonTmp_IsoTk_FittedVtxjpsi;
						std::cout << "Dimuon 1 isolation variable is set to: " << diMuonC_IsoTk_FittedVtxjpsi << std::endl;
					}
					if ( i == 2 ) {
						diMuonF_IsoTk_FittedVtxjpsi = diMuonTmp_IsoTk_FittedVtxjpsi;
						std::cout << "Dimuon 2 isolation variable is set to: " << diMuonF_IsoTk_FittedVtxjpsi << std::endl;
					}
				}
				std::cout << "Filling isolation variables: " << diMuonC_IsoTk_FittedVtxjpsi << std::endl;

				jpsi_isoTk_dimuon1 = diMuonC_IsoTk_FittedVtxjpsi;
				jpsi_isoTk_dimuon2 = diMuonF_IsoTk_FittedVtxjpsi;
				std::cout << "Filling isolation nTuple.  Dimuon 1: " << jpsi_isoTk_dimuon1 << std::endl;
				std::cout << "Filling isolation nTuple.  Dimuon 2: " << jpsi_isoTk_dimuon2 << std::endl;
				b_isoC_1mmjpsi = b_diMuonC_IsoTk_FittedVtxjpsi;
				b_isoF_1mmjpsi = b_diMuonF_IsoTk_FittedVtxjpsi;
				//} //Redunand loop
			}//isolation loop
			if ( m_debug > 10 ) std::cout << m_events << " Stop RECO Level" << std::endl;
		}else{ //Jpsi trigger loop.  Initialize variables that cannot otherwise be reached.
		...
		if(jpsi_event_failure_type == 0) std::cout << "Jpsi event terminated: HLT_Dimuon0_Jpsi_Muon_v2 not fired" << std::endl;
		if(jpsi_event_failure_type == 1) std::cout << "Jpsi event terminated: Event does not contain >= 4 muonts with pT > 3.5" << std::endl;
		if(jpsi_event_failure_type == 2) std::cout << "Jpsi event terminated: Event does not contain 2 mu jets" << std::endl;
		if(jpsi_event_failure_type == 3) std::cout << "Jpsi event terminated: Event does not contain 2 dimuons" << std::endl;
		if(jpsi_event_failure_type == 4) std::cout << "Jpsi event terminated: Both dimuons do not have a valid vertex" << std::endl;
		if(jpsi_event_failure_type == 5) std::cout << "Jpsi event terminated: Event does not pass dz cut (nTuple filled.  Isolation not calculated)" << std::endl;
		if(jpsi_event_failure_type == 6) std::cout << "Jpsi event completed successfully!" << std::endl;
		std::cout << "Jpsi estimation complete" << std::endl;
	} //Jpsi estimation loop

	//****************************************************************************
	//                          PROMPT JPSI ESTIMATION END  
	//****************************************************************************
...
void CutFlowAnalyzer::beginJob() {
	...
	if(runPromptJpsiEstimation_){
		...
		m_ttree_jpsi->Branch("jpsi_mass_dimuon1", &jpsi_mass_dimuon1, "jpsi_mass_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_mass_dimuon2", &jpsi_mass_dimuon2, "jpsi_mass_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_inv_mass_dimuon1", &jpsi_inv_mass_dimuon1, "jpsi_inv_mass_dimuon1/F");  
		m_ttree_jpsi->Branch("jpsi_inv_mass_dimuon2", &jpsi_inv_mass_dimuon2, "jpsi_inv_mass_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_isoTk_dimuon1", &jpsi_isoTk_dimuon1, "jpsi_isoTk_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_isoTk_dimuon2", &jpsi_isoTk_dimuon2, "jpsi_isoTk_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_Lxy_dimuon1", &jpsi_Lxy_dimuon1, "jpsi_Lxy_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_Lxy_dimuon2", &jpsi_Lxy_dimuon2, "jpsi_Lxy_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_L_dimuon1", &jpsi_L_dimuon1, "jpsi_L_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_L_dimuon2", &jpsi_L_dimuon2, "jpsi_L_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_px_dimuon1", &jpsi_px_dimuon1, "jpsi_px_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_px_dimuon2", &jpsi_px_dimuon2, "jpsi_px_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_py_dimuon1", &jpsi_py_dimuon1, "jpsi_py_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_py_dimuon2", &jpsi_py_dimuon2, "jpsi_py_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_pz_dimuon1", &jpsi_pz_dimuon1, "jpsi_pz_dimuon1/F");
		m_ttree_jpsi->Branch("jpsi_pz_dimuon2", &jpsi_pz_dimuon2, "jpsi_pz_dimuon2/F");
		m_ttree_jpsi->Branch("jpsi_trigger", &jpsi_trigger, "jpsi_trigger/I");
	}
}
